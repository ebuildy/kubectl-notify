...
Define the input side of the hexagon for ingesting events that can trigger
notifications. This capability provides a technology-agnostic `EventSource` port
(with an `Event` value object, an `Observer`, and a generic `Filter`) and a
Kubernetes adapter that watches `core/v1` Events and maps them onto the port,
honoring the standard kubectl connection flags.
